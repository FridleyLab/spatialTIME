...
modes_stroma <- function(k, xmin = 0, xmax = 10, ymin = 0, ymax = 10, sdmin = 1/2, sdmax = 2){ 
  ...
  center_peak <- center_peak %>% 
    dplyr::rowwise() %>%
    dplyr::mutate(rho = runif(1, -sd_x * sd_y, sd_x * sd_y))
...
hole <- function(xmin = xmin, xmax = xmax, ymin = ymin, 
  ...
  percent_area_missing <- runif(1, 0.2, 0.35)
  ...
  center_hole <- data.frame(x = runif(num_holes, xmin, xmax),
                            y = runif(num_holes, ymin, ymax),
                            # rho = 0,
                            sd_x = runif(num_holes, sdmin, sdmax), 
                            sd_y = runif(num_holes, sdmin, sdmax))
...
dist_function <- function(a){
  diff <- c((pp$x[i] - center_peak[a,1]), (pp$y[i] - center_peak[a,2]))
  diff <- unlist(diff)
  sigma <- matrix(c(center_peak$sd_x[a]^2, rep(center_peak$rho[a], 2),
                    center_peak$sd_y[a]^2), nrow = 2, ncol = 2)
