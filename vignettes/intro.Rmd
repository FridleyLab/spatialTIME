...
For this example, we use the same data used above for plotting cell locations. Now we only focus on the same TMA core and only focus on CD3+ positive cells. In this particular TMA core that the overall cell distribution is not uniform (i.e. there are many regions where cells do not occur). Also notice, that there is the CD3+ occur mostly in two places thus we suspect that the degree of clustering is quite large in this TMA core.
...
Below is an example that shows how to select a single value for $r$, and how one can visualize the permutation distribution and how it relates to the theoretical and observed value. Here we plot the permutation distribution for the CD3+ cells with $r = 50$ and note that the range of r that is included must included 0 and the output will not include the degree of clustering at $r = 0$ which is 0 for $K$ and $L$, and $\infty$ for $M$. Notice that the theoretical value is smaller than all 100 values from the permutation distribution. 

```{r, fig.width=10, fig.height=6, fig.align = 'center', fig.cap = 'Histogram displaying how much the permuted estimates of K (100 permutations) can vary and also illustrate how different the permutation distribution and its mean (green vertical line) can be from the theoretical value (red vertical line). Lastily, this plot confirms what we see visually above in that the CD3+ cells are not uniformly distributed throughout this side but clustered.'}
...
Once the degree of spatial clustering has be computed questions like "Are CD3+ cells have a larger degree of clustering different between two populations?" (t-test) or "Is the pattern of degree of clustering different between two populations?" (can be checked visually with a heatmap).
