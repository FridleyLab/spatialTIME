...
mif <- create_mif(
  clinical_data = janitor::clean_names(example_clinical),
  sample_data = janitor::clean_names(example_summary),
  spatial_list = lapply(example_spatial, janitor::clean_names),
...
mif <- plot_immunoflo(mif, 
                       plot_title = "deidentified_sample",  
                       mnames = mnames_bad,
                   cell_type = "Classifier.Label")

mif[["derived"]][["spatial_plots"]][[1]]
```


```{r}
bad_names <- plot[["derived"]][["spatial_plots"]][[4]] + 
...
plot <- plot_immunoflo(mif, 
                       plot_title = "deidentified_sample", 
                       mnames = mnames_good, 
                       cell_type = "Classifier.Label")

good_names <- plot[["derived"]][["spatial_plots"]][[4]] + 
  theme(legend.position = 'bottom') + 
