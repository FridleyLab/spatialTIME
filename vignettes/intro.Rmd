...
* Spatial list (1 per each core):
  - This object should be a list object where each element of the list corresponds to a core and each element should be a  $n\times p$ dataframe ($n$ = number of cells)  containing:
    ...
    2. `x_min`, `x_max`, `y_min`, and `y_max` which defines the area that a cell occupies which is eventually used to assign a location for each cell with the x-position being the mean of the `x_min` and `x_max` and y-position being the mean of the `y_min` and `y_max`, and
...
mif <- plot_immunoflo(mif,
  plot_title = "deidentified_sample", mnames = mnames_bad,
  cell_type = "classifier_label"
)

bad_names <- mif[["derived"]][["spatial_plots"]][[4]] +
...
mnames_good <- c("cd3_opal_570_positive", "cd8_opal_520_positive",
                 "foxp3_opal_620_positive", "pdl1_opal_540_positive",
                 "pd1_opal_650_positive", "cd3_cd8", "cd3_foxp3")

mif <- plot_immunoflo(mif,
  plot_title = "deidentified_sample", mnames = mnames_good,
  cell_type = "classifier_label"
)
...
values <- RColorBrewer::brewer.pal(length(unique(mif$derived$univariate_Count[[mif$sample_id]])), "Accent")
names(values) <- unique(mif$derived$univariate_Count$deidentified_sample)

mif$derived$univariate_Count %>%
  filter(Marker != "pdl1_opal_540_positive") %>%
  ggplot(aes(x = r, y = `Degree of Clustering Permutation`)) +
  geom_line(aes(color = get(mif$sample_id)), show.legend = FALSE) +
...
mif$derived$univariate_Count %>%
  filter(Marker == "cd3_cd8.") %>%
  inner_join(mif$clinical, .) %>%
  ggplot(aes(shape = status, y = `Degree of Clustering Exact`, x = r)) +
  geom_point(aes(color = get(mif$sample_id))) +
  theme_bw() +
  scale_color_manual(values = values)
```
...
mif$derived$bivariate_Count %>%
  ...
  ggplot(aes(x = r, y = `Degree of Clustering Permutation`)) +
  geom_line(aes(color = get(mif$sample_id)), show.legend = TRUE) +
  theme_bw() +
  scale_color_manual(values = values)
```
...
mif$derived$univariate_NN %>%
  filter(Marker != "pdl1_opal_540_positive") %>%
  ggplot(aes(x = r, y = `Degree of Clustering Permutation`)) +
  geom_line(aes(color = get(mif$sample_id))) +
  facet_wrap(Marker ~ ., scales = "free") +
  theme_bw() +
  scale_color_manual(values = values)
```

The interpretation of the degree of clustering for $G$ that values greater than 0 indicate spatial clustering of the cell types of interest, while values less than 0 indicate dispersion of these cells. For example in core TMA2_[3,B], `foxp3_opal_620_positive` shows spatial dispersion from $0\le r \le50$ (less than zero), while `cd3_opal_570_positive` for core TMA2_[3,B] shows spatial clustering $0\le r\le75$ (greater than zero).
...
```{r,fig.width=10, fig.height=6, fig.align = 'center'}
mif <- bi_NN_G(
  mif = mif, mnames = c("cd3_cd8", "cd3_foxp3"), num_permutations = 10,
  edge_correction = "rs", r = 0:100, workers = 1, overwrite = TRUE
)

mif$derived$bivariate_NN %>%
  filter(Anchor == "cd3_foxp3") %>%
  ggplot(aes(x = r, y = `Degree of Clustering Permutation`)) +
...
mif <- plot_immunoflo(mif,
  plot_title = "deidentified_sample", mnames = mnames_good[1],
  cell_type = "classifier_label"
)
...
perm_mean <- mif$derived$univariate_Count %>%
  filter(
    deidentified_sample == "TMA3_[9,K].tif",
    r == 50
  ) %>%
...
table(mif$spatial[[1]]$classifier_label)
table(x_tumor$spatial[[1]]$classifier_label)
```
...
inner_join(mif$clinical, mif$derived$univariate_Count) %>%
  filter(grepl("cd3_o|cd8_o", Marker), r == 50) %>%
...
Rip_K_df <- mif$derived$univariate_Count %>%
  filter(!(Marker %in% c(
    "pdl1_opal_540_positive",
    "cd3_foxp3",
    "pd1_opal_650_positive"
  )), r == 50) %>%
  select(deidentified_sample, Marker, `Degree of Clustering Permutation`) %>%
  pivot_wider(
    names_from = deidentified_sample, values_from = `Degree of Clustering Permutation`,
    id_cols = Marker
  )
...
annotation <- inner_join(mif$clinical, mif$derived$univariate_Count) %>%
...
If `xloc` and `yloc` are set as `NULL`, when computing spatial clustering than the cell location is the center of the cell (x-coordinate is the average of `x_min` and `x_max`, and y-coordinate is the average of `y_min` and `y_max`). If you desire to use custom points, this is possible by identifying which column the new coordinates are from. Here were compute Ripley's $K$ with the minimum coordinate of x and y.  

```{r}
mif <- ripleys_k(
  mif = mif, mnames = mnames_good[1], num_permutations = 100,
  edge_correction = "translation", r = c(0, 50), permute = TRUE,
  keep_permutation_distribution = TRUE, workers = 1, overwrite = TRUE,
  xloc = "x_min", yloc = "y_min"
)
```
